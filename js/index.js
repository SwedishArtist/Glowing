...
(() => {
  ...
  let firstX = context.witdh * Math.random();
  let firstY = context.height * Math.random();
  // Färg väljs ut "färghjul", första värdet utgår från detta
  let initHue = 0;
  ...
  * @param curX Där kurvan ska börja = där den slutade sist
  * @param hue Senast använda nyans (HSL)
  */
  function line(curX, curY, hue) {
    ...
    // Gå till slumpmässigt antal grader framåt i färghjulet
    hue += 10 * Math.random();
    //console.log(hue);
    context.strokeStyle = 'hsl(' + ', 50%, 50%';
    // Uppgift (längre fram) - flytta ut shadowColor och shadowBlur
    // eftersom de aldrig ändras
    // Glödande kanlinje på kurvan
    ...
    context.restore();
    // Rekursiv anrop = anropa sig själv
